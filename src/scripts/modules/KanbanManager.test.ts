...
import { KanbanManager, KanbanStatus, AIProcessingState, KanbanConfig } from './KanbanManager';
...
describe('KanbanManager', () => {
  ...
  beforeEach(() => {
    ...
    (GitHubProjectsIntegration as any).mockImplementation(() => mockGitHubProjects);
  ...
  describe('status updates', () => {
    ...
    it('should move issue to In Progress when processing starts', async () => {
      ...
      expect(mockGitHubProjects.updateCardStatus).toHaveBeenCalledWith('card-123', KanbanStatus.IN_PROGRESS);
    ...
    it('should move issue to In Review when PR is generated', async () => {
      ...
      expect(mockGitHubProjects.updateCardStatus).toHaveBeenLastCalledWith('card-123', KanbanStatus.IN_REVIEW);
    ...
    it('should move issue to Done when processing completes', async () => {
      ...
      expect(mockGitHubProjects.updateCardStatus).toHaveBeenLastCalledWith('card-123', KanbanStatus.DONE);
    ...
    it('should move issue back to Backlog when processing fails', async () => {
      ...
      expect(mockGitHubProjects.updateCardStatus).toHaveBeenLastCalledWith('card-123', KanbanStatus.BACKLOG);
  ...
  describe('bidirectional sync', () => {
    ...
    it('should handle sync errors gracefully', async () => {
      mockGitHubProjects.getAllCards.mockRejectedValue(new Error('Sync failed'));

      // Should not throw
      await expect(kanbanManager.checkForManualChanges()).resolves.not.toThrow();
    });
  });

  describe('status mapping', () => {
    it('should map AI states to kanban statuses correctly', () => {
      expect(kanbanManager['mapAIStateToKanban'](AIProcessingState.TODO)).toBe(KanbanStatus.BACKLOG);
      expect(kanbanManager['mapAIStateToKanban'](AIProcessingState.ONGOING)).toBe(KanbanStatus.IN_PROGRESS);
      expect(kanbanManager['mapAIStateToKanban'](AIProcessingState.COMPLETED)).toBe(KanbanStatus.DONE);
    });

    it('should map kanban statuses to AI states correctly', () => {
      expect(kanbanManager['mapKanbanToAIState'](KanbanStatus.BACKLOG)).toBe(AIProcessingState.TODO);
      expect(kanbanManager['mapKanbanToAIState'](KanbanStatus.IN_PROGRESS)).toBe(AIProcessingState.ONGOING);
      expect(kanbanManager['mapKanbanToAIState'](KanbanStatus.IN_REVIEW)).toBe(AIProcessingState.ONGOING);
      expect(kanbanManager['mapKanbanToAIState'](KanbanStatus.DONE)).toBe(AIProcessingState.COMPLETED);
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should handle API errors during status updates', async () => {
      ...
      await expect(kanbanManager.onProcessingStart(123, 'workflow-123')).rejects.toThrow('API Error');
    });

    it('should handle missing issue mappings', async () => {
      await expect(kanbanManager.onPRGenerated(999, 'workflow-999')).rejects.toThrow('KanbanManager not initialized');
    });
  });
