...
import { AutonomousResolver, CodeChanges, ResolutionResult } from './AutonomousResolver';
...
describe('AutonomousResolver', () => {
  ...
  describe('resolveIssue', () => {
    ...
    it('should respect maximum iterations limit', async () => {
      ...
      mockAgent.query.mockResolvedValue(
        JSON.stringify({
          files: [{ path: 'test.ts', changes: [{ type: 'add', content: 'invalid code' }] }],
          isValid: false,
    ...
    it('should handle timeout correctly', async () => {
      ...
      mockAgent.query.mockImplementation(
        () => new Promise(resolve => setTimeout(() => resolve(JSON.stringify({ files: [] })), 100))
      );

      await expect(fastResolver.resolveIssue(mockAnalysis, mockIssue)).rejects.toThrow(
        'Resolution timeout'
      );
    });
    ...
    it('should run safety checks and reject dangerous changes', async () => {
      ...
      mockAgent.query.mockResolvedValue(JSON.stringify({ files: dangerousSolution.files }));
    ...
    it('should enforce file modification limits', async () => {
      ...
      largeResolver['generateSolution'] = vi.fn().mockResolvedValue(tooManyFiles);
    ...
    it('should validate file extensions', async () => {
      ...
      mockAgent.query.mockResolvedValue(JSON.stringify({ files: invalidExtension.files }));
  ...
  describe('analyzeCodebase', () => {
    it('should analyze project structure', async () => {
      const analysis = await (resolver as any).analyzeCodebase('/tmp/test-project');
  ...
  describe('containsDangerousPatterns', () => {
    it('should detect dangerous patterns', () => {
      ...
      expect((resolver as any).containsDangerousPatterns(dangerousCode)).toBe(true);
    });
  ...
  describe('improveSolution', () => {
    it('should improve solution based on validation feedback', async () => {
      ...
      mockAgent.query.mockResolvedValue(
        JSON.stringify({
          files: [
            {
              path: 'src/LoginForm.ts',
              changes: [{ type: 'modify', content: 'return email.includes("@");' }],
            },
          ],
        })
      );
      ...
      expect(improved.files[0].changes[0].content).toBe('return email.includes("@");');
    });
    ...
    it('should return current solution when improvement fails', async () => {
      ...
      const improved = await (resolver as any).improveSolution(
        ...
        { structure: {}, patterns: {}, dependencies: {}, testFiles: [], entryPoints: [] },
        mockIssue
      );
