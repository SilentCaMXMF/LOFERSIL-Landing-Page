...
export const TEST_ANALYSES: Record<string, OpenCodeAnalysis> = {
  simpleBugAnalysis: {
    ...
    acceptanceCriteria: [
      "Button is properly styled on mobile",
      "No layout issues",
    ],
    feasible: true,
    confidence: 0.95,
  ...
  complexFeatureAnalysis: {
    ...
    acceptanceCriteria: [
      "Complete auth system functional",
...
export const TEST_SOLUTIONS: Record<string, OpenCodeSolution> = {
  ...
  securitySolution: {
    ...
    explanation:
      "Fixed SQL injection by using parameterized queries and added input sanitization",
    testCases: [
...
export function getAnalysisByType(
  type: keyof typeof TEST_ANALYSES,
): OpenCodeAnalysis {
  return TEST_ANALYSES[type];
}

export function getSolutionByType(
  type: keyof typeof TEST_SOLUTIONS,
): OpenCodeSolution {
  return TEST_SOLUTIONS[type];
}

export function getReviewByType(
  type: keyof typeof TEST_REVIEWS,
): OpenCodeReviewResult {
...
export function getTestIssuesByLabel(label: string): GitHubIssue[] {
  return Object.values(TEST_ISSUES).filter((issue) =>
...
export function getTestIssuesByComplexity(complexity: string): GitHubIssue[] {
  ...
  return issueKeys
    .map((key) => TEST_ISSUES[key as keyof typeof TEST_ISSUES])
    .filter(Boolean);
}
