...
export function createMockGitHubPR(overrides: Partial<GitHubPR> = {}): GitHubPR {
...
export class GitHubAPIMock {
  ...
  // Mock API Methods
  issues = {
    get: vi.fn().mockImplementation(async ({ issue_number }: { issue_number: number }) => {
    ...
    update: vi
      .fn()
      .mockImplementation(
        async ({ issue_number, state }: { issue_number: number; state: string }) => {
          await this.simulateLatency();
  ...
  pulls = {
    ...
    update: vi
      .fn()
      .mockImplementation(
        async ({ pull_number, state }: { pull_number: number; state: string }) => {
          await this.simulateLatency();
...
export function createGitHubAPIMock(
  ...
  if (config) {
    ...
    if (config.responseLatency !== undefined) mock.setLatency(config.responseLatency);
  }
  return mock;
}
