...
describe('PRGenerator', () => {
  ...
  describe('createPullRequest', () => {
    it('should create a complete pull request successfully', async () => {
      ...
      const result = await generator.createPullRequest(mockResolution, mockReview, mockIssue);
    ...
    it('should assign appropriate reviewers based on change scope', async () => {
          ...
          files: [
            {
              ...
              changes: [{ type: 'modify', content: 'const component = () => <div/>;' }],
            },
          ],
        },
      };
      ...
      expect(generator['createGitHubPR']).toHaveBeenCalledWith(
        expect.objectContaining({
          reviewers: expect.arrayContaining(['default-reviewer', 'frontend-lead']),
        })
      );
    });
    ...
    it('should add security reviewers for security issues', async () => {
      ...
      await generator.createPullRequest(mockResolution, securityReview, mockIssue);
  ...
  describe('generatePRTitle', () => {
    it('should generate meaningful PR titles using AI', async () => {
      mockAgent.query.mockResolvedValue('feat(auth): implement robust email validation');

      const title = await generator['generatePRTitle'](mockResolution, mockReview, mockIssue);

      expect(title).toBe('feat(auth): implement robust email validation');
      expect(mockAgent.query).toHaveBeenCalledWith(
        expect.stringContaining('Generate a concise, meaningful pull request title'),
        expect.any(Object)
      );
    });
  ...
  describe('generatePRDescription', () => {
    it('should create comprehensive PR descriptions', async () => {
      ...
      expect(description).toContain('**Review Status:** ✅ Approved');
  ...
  describe('determineLabels', () => {
    it('should include auto-generated label', () => {
      const labels = generator['determineLabels'](mockResolution, mockReview, mockIssue);
    ...
    it('should add needs-review label for unapproved PRs', () => {
      ...
      const labels = generator['determineLabels'](mockResolution, unapprovedReview, mockIssue);
    ...
    it('should add security label for security issues', () => {
      ...
      const labels = generator['determineLabels'](mockResolution, securityReview, mockIssue);
    ...
    it('should add size labels based on PR size', () => {
      ...
      const labels = generator['determineLabels'](largeResolution, mockReview, mockIssue);
  ...
  describe('createCommits', () => {
    it('should create commits from changes', async () => {
      ...
      const commits = await generator['createCommits'](mockResolution.solution, 'test-branch');
      ...
      expect(generator['gitAdd']).toHaveBeenCalledWith('src/components/LoginForm.ts');
  ...
  describe('utility methods', () => {
    ...
    it('should infer issue types correctly', () => {
      expect(generator['inferIssueType']({ number: 1, title: 'Fix bug', body: '' })).toBe('bug');
      expect(generator['inferIssueType']({ number: 1, title: 'Add feature', body: '' })).toBe(
        'feature'
      );
      expect(generator['inferIssueType']({ number: 1, title: 'Update docs', body: '' })).toBe(
        'documentation'
      );
    });

    it('should calculate PR size correctly', () => {
      const smallPR = generator['calculatePRSize']({ files: [{ changes: [{}] }] });
      const mediumPR = generator['calculatePRSize']({ files: Array(5).fill({ changes: [{}] }) });
      const largePR = generator['calculatePRSize']({ files: Array(15).fill({ changes: [{}] }) });
  ...
  describe('PR creation with various change types', () => {
    it('should create PR for documentation changes', async () => {
      ...
      const result = await generator.createPullRequest(docsResolution as any, mockReview, docsIssue);
    ...
    it('should create PR for feature changes', async () => {
      ...
      const result = await generator.createPullRequest(mockResolution, mockReview, featureIssue);
  ...
  describe('label inference for different scenarios', () => {
    ...
    it('should add size labels for medium PRs', () => {
      ...
      const labels = generator['determineLabels'](mediumResolution, mockReview, mockIssue);
    ...
    it('should handle multiple label conditions', () => {
      ...
      const labels = generator['determineLabels'](breakingResolution, complexReview, mockIssue);
  ...
  describe('GitHub API error handling', () => {
    it('should handle GitHub API failures gracefully', async () => {
      mockAgent.query.mockResolvedValueOnce('feat: test title').mockResolvedValueOnce('Test body');

      generator['createGitHubPR'] = vi.fn().mockRejectedValue(new Error('GitHub API rate limit exceeded'));
    ...
    it('should handle network errors in GitHub API', async () => {
      ...
      generator['createGitHubPR'] = vi.fn().mockRejectedValue(new Error('Network timeout'));
  ...
  describe('error handling', () => {
    it('should handle PR creation failures', async () => {
      ...
      generator['createGitHubPR'] = vi.fn().mockRejectedValue(new Error('GitHub API error'));
    ...
    it('should handle commit creation failures', async () => {
      generator['gitCommit'] = vi.fn().mockRejectedValue(new Error('Git commit failed'));
