...
describe('IssueAnalyzer', () => {
  ...
  describe('analyzeIssue', () => {
    it('should analyze a bug issue successfully', async () => {
      ...
      mockAgent.query
        ...
        .mockResolvedValueOnce(
          JSON.stringify({
            ...
            acceptanceCriteria: ['Invalid emails are rejected', 'Error message is displayed'],
          })
      ...
      expect(result.acceptanceCriteria).toContain('Invalid emails are rejected');
    ...
    it('should categorize issues by labels when AI fails', async () => {
      ...
      mockAgent.query.mockRejectedValueOnce(new Error('AI service unavailable'));
    ...
    it('should handle AI response parsing errors gracefully', async () => {
      ...
      mockAgent.query.mockResolvedValueOnce('bug').mockResolvedValueOnce('Invalid JSON response');
  ...
  describe('categorizeByLabels', () => {
    ...
    it('should return unknown for uncategorized labels', () => {
      const categories = (analyzer as any).categorizeByLabels([{ name: 'random-label' }]);
  ...
  describe('assessComplexity', () => {
    it('should assess low complexity for simple issues', () => {
      const complexity = (analyzer as any).assessComplexity(mockIssue, mockIssue.body);
      expect(complexity).toBe('low');
    });

    it('should assess high complexity for issues with code blocks', () => {
      const complexBody = '```\ncode block 1\n```\n```\ncode block 2\n```\n```\ncode block 3\n```';
      const complexity = (analyzer as any).assessComplexity(mockIssue, complexBody);
    ...
    it('should assess high complexity for major labeled issues', () => {
      ...
      const complexity = (analyzer as any).assessComplexity(majorIssue, majorIssue.body);
  ...
  describe('isAutonomousFeasible', () => {
    it('should reject critical complexity issues', () => {
      const feasible = (analyzer as any).isAutonomousFeasible('critical', 'bug', []);
      expect(feasible).toBe(false);
    });

    it('should reject question-type issues', () => {
      const feasible = (analyzer as any).isAutonomousFeasible('low', 'question', []);
      expect(feasible).toBe(false);
    });

    it('should reject unknown category issues', () => {
      const feasible = (analyzer as any).isAutonomousFeasible('low', 'unknown', []);
      expect(feasible).toBe(false);
    });

    it('should accept simple bug fixes', () => {
      const feasible = (analyzer as any).isAutonomousFeasible('low', 'bug', ['Fix typo']);
      expect(feasible).toBe(true);
    });

    it('should reject high complexity features', () => {
      const feasible = (analyzer as any).isAutonomousFeasible('high', 'feature', []);
      expect(feasible).toBe(false);
    });
  });
  ...
  describe('calculateConfidence', () => {
    it('should calculate high confidence for clear issues', () => {
      const confidence = (analyzer as any).calculateConfidence('bug', 'low', ['Clear requirement']);
      expect(confidence).toBeGreaterThan(0.7);
    });

    it('should calculate low confidence for unknown categories', () => {
      const confidence = (analyzer as any).calculateConfidence('unknown', 'high', []);
      expect(confidence).toBeLessThan(0.5);
    });

    it('should boost confidence with requirements', () => {
      const confidence = (analyzer as any).calculateConfidence('bug', 'medium', ['Req 1', 'Req 2']);
