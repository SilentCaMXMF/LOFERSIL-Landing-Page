...
export class IssueAnalyzer {
  ...
  async analyzeIssue(issue: GitHubIssue): Promise<IssueAnalysis> {
    ...
    try {
      ...
      const { requirements, acceptanceCriteria } =
        await this.extractRequirements(content, category);

      // Determine feasibility for autonomous resolution
      const feasible = this.isAutonomousFeasible(
        complexity,
        category,
        requirements,
      );
...
Respond with only the category name in lowercase.`;
    ...
    try {
      ...
      const category = response
        .trim()
        .toLowerCase() as IssueAnalysis["category"];
      ...
      console.warn(
        "AI categorization failed, falling back to label-based categorization",
      );
    }
  ...
  private assessComplexity(
    ...
    const filePathCount = (
      content.match(/(?:^|\s)(?:src|lib|test|docs)\/[^\s]+/g) || []
    ).length;
...
Acceptance criteria should be the conditions that must be met for the issue to be considered resolved.`;
      ...
      if (
        ...
        if (
          ...
          acceptanceCriteria.push(
            itemContent.replace(/^acceptance:\s*/i, "").trim(),
          );
        } else if (
  ...
  private generateReasoning(
    ...
    reasons.push(
      `Autonomous resolution ${feasible ? "feasible" : "not feasible"}`,
    );
    reasons.push(`Analysis confidence: ${Math.round(confidence * 100)}%`);

    if (!feasible) {
      if (complexity === "critical")
        reasons.push("Critical complexity requires human review");
      if (category === "question")
        reasons.push("Questions need human clarification");
      if (category === "unknown")
        reasons.push("Unable to determine issue type");
    }
  ...
  private extractContent(issue: GitHubIssue): string {
    ...
    const sanitizedTitle = this.domPurify.sanitize(issue.title, {
      ALLOWED_TAGS: [],
    });
    const sanitizedBody = this.domPurify.sanitize(issue.body || "", {
      ALLOWED_TAGS: [],
    });
