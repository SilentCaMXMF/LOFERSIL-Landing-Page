...
export function createMockAnalysis(
  overrides: Partial<OpenCodeAnalysis> = {},
): OpenCodeAnalysis {
...
export function createMockSolution(
  overrides: Partial<OpenCodeSolution> = {},
): OpenCodeSolution {
...
export class OpenCodeAgentMock {
  ...
  analyze = vi
    .fn()
    .mockImplementation(async (issue: any): Promise<OpenCodeAnalysis> => {
  ...
  generateCode = vi
    .fn()
    .mockImplementation(
      async (analysis: OpenCodeAnalysis): Promise<OpenCodeSolution> => {
        await this.simulateLatency();
  ...
  reviewCode = vi
    .fn()
    .mockImplementation(
      async (solution: OpenCodeSolution): Promise<OpenCodeReviewResult> => {
        await this.simulateLatency();
        ...
        const review = createMockReviewResult({
          ...
          securityIssues:
            qualityScore > 0.9 ? [] : ["Potential security concern"],
        });
...
export function createOpenCodeAgentMock(
  ...
  if (config) {
    ...
    if (config.analysisLatency !== undefined)
      mock.setLatency("analysis", config.analysisLatency);
    if (config.generationLatency !== undefined)
      mock.setLatency("generation", config.generationLatency);
    if (config.reviewLatency !== undefined)
      mock.setLatency("review", config.reviewLatency);
    if (config.qualityScoreRange)
      mock.setQualityScoreRange(
        config.qualityScoreRange[0],
        config.qualityScoreRange[1],
      );
  }
  return mock;
}
