...
async function makeMCPRequest(method, params = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(url, requestOptions, (res) => {
      console.log(`📊 MCP Response status: ${res.statusCode} ${res.statusMessage}`);
    ...
    console.log(`📤 Sending MCP request:`, JSON.stringify(requestData, null, 2));
...
async function testBasicConnectivity() {
  console.log('\n🔍 Testing Basic HTTP Connectivity to Context7...');
  
  try {
    const response = await makeHttpRequest(CONTEXT7_CONFIG.baseUrl);
    ...
    if (response.statusCode === 200) {
      ...
      console.log(`❌ Basic connectivity test FAILED: Status ${response.statusCode}`);
      console.log('📄 Response data:', JSON.stringify(response.data, null, 2));
      return false;
    }
  } catch (error) {
...
async function testMCPProtocol() {
  ...
  try {
    ...
    if (initResponse.statusCode === 200 && initResponse.data.result) {
      ...
      console.log('📄 Initialize response:', JSON.stringify(initResponse.data.result, null, 2));
    } else {
  ...
  try {
    ...
    if (resolveTool) {
      ...
      if (response.statusCode === 200 && response.data.result) {
        ...
        console.log('📄 Tool response:', JSON.stringify(response.data.result, null, 2));
...
async function runTests() {
  ...
  console.log(`  Basic Connectivity: ${results.connectivity ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`  MCP Protocol: ${results.mcpProtocol ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`  Tools Discovery: ${results.tools.length > 0 ? `✅ PASSED (${results.tools.length} tools)` : '❌ FAILED'}`);
  console.log(`  Tool Calls: ${results.toolCalls ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`  Documentation: ${results.documentation ? '✅ PASSED' : '❌ FAILED'}`);
  
  const allPassed = results.connectivity && results.mcpProtocol && results.tools.length > 0;
  
  if (allPassed) {
    console.log('\n🎉 ALL TESTS PASSED! Context7 HTTP transport is working correctly.');
    console.log('📋 Ready for Phase 2 implementation with updated documentation from Context7.');
  } else {
...
export { runTests, testBasicConnectivity, discoverTools, testMCPRequest: makeMCPRequest };