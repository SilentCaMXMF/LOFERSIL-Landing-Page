...
export class SEOMetrics {
    constructor() {
        ...
        this.accessibilityMetrics = null;
        this.seoPracticesMetrics = null;
        this.performanceIssues = [];
        this.clsScore = 0;
        this.hasInputOccurred = false;
        this.fidValue = null;
    ...
    calculateSEOScore() {
        try {
            ...
            // Weighted average
            const overall = Math.round(coreWebVitalsScore * 0.4 +
                mobileScore * 0.2 +
     ...
     */
    trackPerformanceIssue(issue, details) {
        const perfIssue = {
            type: issue,
    ...
    checkAccessibility() {
        try {
            ...
            const accessibilityScore = maxIssues > 0
    ...
    checkSEOPractices() {
        try {
            ...
            // Check meta title
            hasMetaTitle = Boolean(document.title &&
            ...
            if (!hasMetaTitle) {
                this.trackPerformanceIssue("Missing or poor page title", {
                    severity: "high",
                    recommendation: "Add a descriptive page title (30-60 characters) including primary keywords",
                });
            }
            if (!hasCanonical) {
     ...
     */
    evaluateMetric(metricName, value, thresholds) {
        let severity = "low";
        let recommendation = "";
        if (value > thresholds.needsImprovement) {
            severity = "high";
        ...
        if (severity !== "low") {
            ...
            this.trackPerformanceIssue(`${metricName.toUpperCase()} is ${metricDisplay} (good: <${thresholdDisplay})`, {
                severity,
                metric: metricName,
                value,
    ...
    generateRecommendations(score) {
        ...
        if (score.coreWebVitals < 70) {
            recommendations.push("Optimize Core Web Vitals to improve page load performance and user experience");
        }
        ...
        if (recommendations.length === 0) {
            recommendations.push("Great job! Your SEO metrics look good. Keep monitoring and optimizing.");
        }
...
//# sourceMappingURL=SEOMetrics.js.map