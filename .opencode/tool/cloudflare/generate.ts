...
import { getMonitor, initializeMonitoring } from './monitoring.js';
...
export async function generateImageWithFlux(config: ImageGenerationConfig): Promise<string> {
  ...
  try {
    ...
    await monitor.recordMetric({
      model: CLOUDFLARE_MODELS.FLUX_SCHNELL,
      operation: 'generate',
      startTime,
      endTime,
      ...
      parameters: { prompt: enhancedPrompt, steps, seed },
    });
...
/**
 * Tool for generating images using Flux-1-Schnell (fast, high quality)
 */
export const generateFlux = tool({
  description:
    'Generate an image using Cloudflare Flux-1-Schnell model (fast, high quality, free tier)',
  args: {
    ...
    steps: tool.schema
      ...
      .optional()
      .default(4)
      .describe('Number of diffusion steps (1-8, default: 4)'),
  ...
  async execute(args, context) {
    try {
      ...
      return `Error generating image with Flux: ${error.message}`;
    }
  },
});

/**
 * Tool for generating images using Stable Diffusion XL Base (high quality)
 */
export const generateStableDiffusionXL = tool({
  description:
    'Generate an image using Cloudflare Stable Diffusion XL Base model (high quality, free tier)',
  args: {
    prompt: tool.schema.string().describe('Text description of the image to generate'),
    negativePrompt: tool.schema.string().optional().describe('Elements to avoid in the image'),
    height: tool.schema
      .number()
      .optional()
      .default(1024)
    ...
    numSteps: tool.schema
      .number()
      .optional()
      .default(20)
      .describe('Number of diffusion steps (1-20, default: 20)'),
  ...
  async execute(args, context) {
    try {
      return await generateImageWithStableDiffusionXL({
        prompt: args.prompt,
      ...
      return `Error generating image with Stable Diffusion XL: ${error.message}`;
    }
  },
});

/**
 * Tool for generating images using Stable Diffusion XL Lightning (fast SDXL)
 */
export const generateStableDiffusionLightning = tool({
  description:
    'Generate an image using Cloudflare Stable Diffusion XL Lightning model (fast SDXL, free tier)',
  args: {
    prompt: tool.schema.string().describe('Text description of the image to generate'),
    negativePrompt: tool.schema.string().optional().describe('Elements to avoid in the image'),
    height: tool.schema
    ...
    numSteps: tool.schema
      ...
      .optional()
      .default(8)
      .describe('Number of diffusion steps (1-20, default: 8)'),
    guidance: tool.schema
      .number()
      .optional()
      .default(7.5)
  ...
  async execute(args, context) {
    try {
      return await generateImageWithStableDiffusionLightning({
        prompt: args.prompt,
        negativePrompt: args.negativePrompt,
        height: args.height,
        width: args.width,
      ...
      return `Error generating image with Stable Diffusion Lightning: ${error.message}`;
    }
  },
});

/**
 * Tool for getting performance metrics
 */
export const getMetrics = tool({
  description: 'Get performance metrics and cost tracking for Cloudflare image generation',
  args: {},
  async execute(args, context) {
    try {
...
export const cloudflareImageTools = [
  generateFlux,
  generateStableDiffusionXL,
  generateStableDiffusionLightning,
  getMetrics,
];

// Default export
export default generateFlux;
