...
describe('MCP Client Header Functionality', () => {
    ...
    describe('Header Masking', () => {
        it('should mask sensitive headers in logs', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('INFO [MCPClient] Initializing MCP connection with headers'), expect.objectContaining({
    ...
    describe('HTTP-based Connection with Headers', () => {
        it('should use headers in HTTP initialization', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith('https://mcp.context7.com/mcp', expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    'Content-Type': 'application/json',
                    CONTEXT7_API_KEY: 'test-api-key',
        ...
        it('should handle HTTP response errors', async () => {
            ...
            await expect(client.connect()).rejects.toThrow('HTTP initialization failed: 401 Unauthorized');
        });
        it('should handle MCP protocol errors in HTTP response', async () => {
            ...
            await expect(client.connect()).rejects.toThrow('MCP initialization error: Authentication failed');
        });
    });
    ...
    describe('Header Security', () => {
        it('should not log sensitive header values in plain text', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('INFO [MCPClient] MCP HTTP initialized successfully'));
            consoleSpy.mockRestore();
        });
