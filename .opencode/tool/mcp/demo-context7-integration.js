...
class OpenCodeDocumentationDemo {
    ...
    async demonstrateMCPFactory() {
        ...
        try {
            ...
            try {
                ...
                console.log('⚠️ Context7 MCP creation failed (expected if env vars not set):', error.message);
            }
        }
        catch (error) {
            console.error('❌ MCP Factory demo failed:', error.message);
        }
    }
    ...
    async demonstrateDocumentationQueries() {
    ...
    async generateDemoReport(connectionSuccess) {
        console.log('\n📊 DEMO SUMMARY REPORT');
