 ...
 */
import { MCPConnectionState, MCPErrors, createMCPError } from './types.js';
...
export class MCPClient {
  ...
  async disconnect() {
    ...
    for (const [id, request] of this.pendingRequests) {
      ...
      const error = createMCPError(MCPErrors.GENERIC_ERROR, 'Connection closed');
  ...
  async sendRequest(method, params) {
    if (this.connectionState !== MCPConnectionState.CONNECTED) {
      const error = createMCPError(MCPErrors.NOT_CONNECTED);
      throw error;
    }
    ...
    if (!this.checkRateLimit()) {
      const error = createMCPError(MCPErrors.RATE_LIMIT_EXCEEDED);
      throw error;
    }
    ...
    if (this.config.headers) {
      ...
      try {
        ...
        if (!response.ok) {
          const error = createMCPError(MCPErrors.HTTP_REQUEST_FAILED);
          throw error;
        }
        const result = await response.json();
        if (result.error) {
          const error = createMCPError(MCPErrors.HTTP_REQUEST_FAILED, undefined, result.error);
          throw error;
        }
      ...
      return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          ...
          const error = createMCPError(MCPErrors.REQUEST_TIMEOUT);
          reject(error);
