...
describe('MCP Configuration Loader', () => {
    ...
    describe('loadConfig', () => {
        ...
        it('should throw error for invalid JSON', () => {
            ...
            expect(() => loadConfig('/path/to/config.json')).toThrow('Failed to load config from /path/to/config.json');
        });
        it('should throw error when file does not exist', () => {
            readFileSync.mockImplementation(() => {
                throw new Error('File not found');
            });
            expect(() => loadConfig('/path/to/config.json')).toThrow('Failed to load config from /path/to/config.json: File not found');
        });
    });
    describe('validateConfig', () => {
        ...
        it('should reject server without type', () => {
            ...
            const result = validateConfig(config);
            expect(result.valid).toBe(false);
            expect(result.errors).toContain('Server "context7": missing or invalid "type" (must be \'remote\' or \'local\')');
        });
    ...
    describe('substituteEnvVars', () => {
        ...
        it('should throw error for undefined environment variables', () => {
            ...
            expect(() => substituteEnvVars(config)).toThrow('Environment variable UNDEFINED_VAR is not set');
        });
    ...
    describe('resolveServerConfig', () => {
        ...
        it('should throw error for non-existent server', () => {
            ...
            expect(() => resolveServerConfig('nonexistent', config)).toThrow('Server "nonexistent" not found in configuration');
        });
        it('should throw error for disabled server', () => {
            ...
            expect(() => resolveServerConfig('context7', config)).toThrow('Server "context7" is disabled');
        });
    });
