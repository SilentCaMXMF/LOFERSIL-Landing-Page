/**
 * Simplified Gemini Tool for Image Generation, Editing, and Analysis
 * Minimal version without advanced features
 */

import {
  tool,
  ToolContext,
} from '../../../.opencode/node_modules/@opencode-ai/plugin/dist/tool.js';
import { getApiKey } from '../env/index.js';
...
/**
 * Configuration options for image generation and editing
 * @typedef {Object} ImageConfig
 * @property {string} [outputDir] - Custom output directory for generated files
 * @property {string} [customName] - Custom filename for the output file
 */
...
function parseImageResponse(json: GeminiResponse): string {
...
function parseTextResponse(json: GeminiResponse): string {
  const text = json?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!text) {
    throw new ValidationError('No analysis returned');
  }
  return text;
}
 ...
 * @param {ImageConfig} [config] - Configuration options for output
 * @returns {Promise<string>} - Path to the generated image file or mock response in test mode
 */
...
/**
 * Edits an existing image using Gemini AI
 * @param {string} imageData - File path or data URL of the image to edit
 * @param {string} prompt - The edit instruction
 * @param {ImageConfig} [config] - Configuration options for output
 * @returns {Promise<string>} - Path to the edited image file or mock response in test mode
 */
export async function editImage(
  ...
  if (isTestMode()) {
    ...
    const originalName = basename(imageData, extname(imageData));
    ...
    return `[TEST MODE] Would edit image: ${imageData} -> ${outputPath} with prompt: "${prompt.substring(0, 50)}..."`;
  }

  const { mime, base64 } = await parseImageInput(imageData);
 ...
 * @param {string} imageData - File path or data URL of the image to analyze
 * @param {string} question - The question to ask about the image
 * @returns {Promise<string>} - Analysis text or mock response in test mode
 */
export async function analyzeImage(imageData: string, question: string): Promise<string> {
  validatePrompt(question);
  const apiKey = await getGeminiApiKey();

  if (isTestMode()) {
    return `[TEST MODE] Would analyze image: ${imageData} with question: "${question.substring(0, 50)}..." - Mock analysis: This is a test image analysis response.`;
  }
...
export const generate = tool({
  description: 'Generate an image using Gemini from text prompt',
  args: {
  ...
  async execute(
    args: { prompt: string; outputDir?: string; filename?: string },
    context: ToolContext
  ) {
    try {
...
export const edit = tool({
  ...
  async execute(
    args: { image: string; prompt: string; outputDir?: string; filename?: string },
    context: ToolContext
  ) {
    try {
      const config: ImageConfig = {
