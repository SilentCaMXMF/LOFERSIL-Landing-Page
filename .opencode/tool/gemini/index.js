...
// Constants
const GEMINI_IMAGE_MODEL_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image-preview:generateContent';
const GEMINI_TEXT_MODEL_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent';
// Error Classes
...
async function makeGeminiApiRequest(url, body, apiKey) {
...
function getDateBasedPath(baseDir) {
    if (!baseDir) {
...
async function getUniqueFilename(directory, baseName, extension, isEdit = false) {
 ...
 */
export async function editImage(imageData, prompt, config = {}) {
    validatePrompt(prompt);
    validateConfig(config);
    const apiKey = await getGeminiApiKey();
    if (isTestMode()) {
