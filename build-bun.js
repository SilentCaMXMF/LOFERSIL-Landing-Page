...
/**
 * LOFERSIL Landing Page Build Script (Bun Version)
...
console.log('🚀 Starting LOFERSIL Landing Page build (Bun runtime)...');
...
// Process CSS with PostCSS using Bun
console.log('🎨 Processing CSS with Bun...');
try {
  if (isProduction) {
    await $`bunx postcss src/styles/main.css -o dist/main.min.css --env production`;
  } else {
    await $`bunx postcss src/styles/main.css -o dist/main.css`;
  }
...
// Minify JavaScript in production using Bun
if (isProduction) {
  console.log('🗜️ Minifying JavaScript with Bun...');
  try {
    await $`bunx terser dist/scripts/index.js -o dist/scripts/index.min.js --compress --mangle --source-map`;
...
console.log(
  `🎉 Build completed successfully in ${isProduction ? 'production' : 'development'} mode using Bun!`
);
