...
import { DEFAULT_TEST_CONFIG, TEST_SCENARIOS } from '../../src/scripts/modules/github-issues/test-config';
...
describe('Performance Benchmarking', () => {
  ...
  describe('Single Workflow Performance', () => {
    it('should complete simple bug fix within performance target', async () => {
      ...
      benchmark.start();
      const result = await orchestrator.processIssue(1, 'Simple bug', 'Fix this bug');
      const metrics = benchmark.end();

      expect(result.success).toBe(true);
      expect(result.finalState).toBe(WorkflowState.PR_COMPLETE);
      expect(metrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.workflowTimeout);
      expect(metrics.avgMemory).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.memory.maxWorkflowMemory);
    });
    ...
    it('should handle complex feature requests within extended time limits', async () => {
      ...
      mocks.issueAnalyzer.analyzeIssue.mockResolvedValue({
        category: 'feature',
        complexity: 'high',
        requirements: ['Implement complex feature', 'Add tests', 'Update documentation'],
      ...
      benchmark.start();
      const result = await orchestrator.processIssue(2, 'Complex feature', 'Implement complex feature');
      const metrics = benchmark.end();

      expect(result.success).toBe(true);
      expect(metrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.workflowTimeout * 1.5); // Allow 50% overhead
      expect(metrics.peakMemory).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.memory.maxHeapUsage);
    });
    ...
    it('should require human review for critical issues without performance penalty', async () => {
      ...
      benchmark.start();
      const result = await orchestrator.processIssue(3, 'Critical issue', 'Security vulnerability');
      const metrics = benchmark.end();

      expect(result.success).toBe(false);
      expect(result.finalState).toBe(WorkflowState.REQUIRES_HUMAN_REVIEW);
      expect(metrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.stepTimeouts.analysis * 2);
    });
  });
  ...
  describe('Concurrent Processing Performance', () => {
    it('should handle multiple concurrent workflows efficiently', async () => {
      ...
      expect(metrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.workflowTimeout * concurrentWorkflows);
      expect(metrics.peakMemory).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.memory.maxHeapUsage);

      // Calculate throughput
      const throughput = (concurrentWorkflows * 1000) / metrics.duration; // workflows per second
      expect(throughput).toBeGreaterThan(DEFAULT_TEST_CONFIG.thresholds.minThroughput / 60); // per second vs per minute
    });
  ...
  describe('Memory Usage Monitoring', () => {
    ...
    it('should clean up resources after workflow completion', async () => {
      ...
      benchmark.start();
      const result = await orchestrator.processIssue(3000, 'Cleanup test', 'Testing resource cleanup');
      const metrics = benchmark.end();
      ...
      expect(memoryIncrease).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.memory.maxWorkflowMemory);
    });
  });
  ...
  describe('Component-Level Performance', () => {
    it('should benchmark individual component execution times', async () => {
      ...
      expect(analysisMetrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.stepTimeouts.analysis);
      ...
      expect(resolutionMetrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.stepTimeouts.resolution);
      ...
      expect(reviewMetrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.stepTimeouts.review);

      // PR generation performance
      componentBenchmarks.pr.start();
      await mocks.prGenerator.createPullRequest(
        { success: true, solution: { description: 'test', files: [], tests: [] }, confidence: 0.8, reasoning: 'test' },
        { approved: true, score: 0.9, comments: [], suggestions: [], criticalIssues: [] },
        {
          category: 'bug',
          complexity: 'low',
          requirements: ['test'],
      ...
      expect(prMetrics.duration).toBeLessThan(DEFAULT_TEST_CONFIG.benchmarks.stepTimeouts.prCreation);
    });
  });
  ...
  describe('Performance Thresholds Validation', () => {
    it('should validate all test scenarios against performance targets', () => {
      TEST_SCENARIOS.forEach(scenario => {
        ...
        expect(scenario.performanceTarget).toBeLessThanOrEqual(DEFAULT_TEST_CONFIG.benchmarks.workflowTimeout);
      });
    });

    it('should ensure performance targets are realistic', () => {
      const fastScenarios = TEST_SCENARIOS.filter(s => s.tags.includes('fast'));
      const complexScenarios = TEST_SCENARIOS.filter(s => s.tags.includes('high-complexity'));
